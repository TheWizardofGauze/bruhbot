...
async def help(ctx, *arg):
    pre=" ".join(arg[:]).lower()
    args = ['addr', 'delr', 'rlist', 'age', 'response']
    if pre in args:
        with open('data.json') as f:
            data = json.load(f)
        ...
        with open('data.json') as f:
            data = json.load(f)
            helpName = data['response']['name']
            helpDes = data['response']['description']
            helpCont = data['response']['content']
            helpName2 = data['extra']['name']
            helpDes2 = data['extra']['description']
            helpCont2 = data['extra']['content']
        helpColor = await get_color(ctx)
        msg = await embed(ctx, helpName, helpDes, helpCont, helpColor)
