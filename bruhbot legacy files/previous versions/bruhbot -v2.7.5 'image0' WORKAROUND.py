                ...
                with open('.\\bruhbot\\responses.txt', 'r') as f:
                    if arg:
                        ...
                        if attachment.filename == f"image0{ext}":
                            new = ''.join(random.choice(string.ascii_letters+string.digits) for i in range(6))
...
@bot.command()
async def delr(ctx):
    def num_list(responses, start, end):
        numbered = []
    ...
    numbered = num_list(responses, start, end)
...
async def help(ctx, *arg):
    pre=" ".join(arg[:]).lower()
    args = ['addr', 'delr', 'rlist', 'age', 'response', 'extra']
