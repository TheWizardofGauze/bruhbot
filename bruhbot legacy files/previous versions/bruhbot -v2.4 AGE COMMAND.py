...
async def embed(name, description, content, color):
    ...
    embed.set_author(name=f'{bot.user.display_name} Help Menu', icon_url=bot.user.avatar_url)
...
async def on_message(ctx):
    ...
    if ctx.guild:
        name = re.sub('[^a-zA-Z0-9]', '', ctx.guild.me.display_name).lower()
    else:
...
async def help(ctx, *arg):
    ...
    if pre == 'addr':
        ...
        msg2 = await embed(helpName2, helpDes2, helpCont2, helpColor)
        user = ctx.author
