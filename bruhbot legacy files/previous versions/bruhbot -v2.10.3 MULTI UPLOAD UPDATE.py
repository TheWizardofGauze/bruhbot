...
async def embed(ctx, name, description, content, color, pages):
    ...
    embed.set_author(
        name=f"{ctx.me.display_name} Help Menu", icon_url=bot.user.avatar_url
    )
    embed.set_footer(
            ...
            for attachment in ctx.message.attachments:
                if "image" in attachment.content_type:
                    ...
                    if len(ctx.message.attachments) == 1:
                        ...
                        counter += 1
            if len(ctx.message.attachments) - counter == 1:
                added = "Image was added"
            else:
                added = "Images were added"
            if not counter == len(ctx.message.attachments):
                await ctx.send(":ok_hand:")
                await ctx.send(added)
            if counter > 0:
                await ctx.send(f"Blocked {str(counter)} invalid files.")
            return
        if not arg:
            await help(ctx, "addr")
            return
    ...
    try:
        ...
        if not ctx.guild:  # dm delete
            ...
            if arg:
                ...
                msg = await ctx.send(
                    f"Do you want to delete **'{responses[choice]}'**?\n(Y/N)"
                )
                while True:
                    try:
                                ...
                                if choice2 == "y":
                                    ...
                                    with open(
                                        ...
                                        for line in file:
                                            if (
                                                line.strip("\n").lower()
                                                != responses[choice].lower()
                                            ):
                                                f.write(line)
                                    if " - image" in responses[choice]:
                                        try:
                                            image = responses[choice].replace(
                                                " - image", ""
                                            )
                                            os.chdir(".\\images")
                                            os.remove(image)
                                    ...
                                    await ctx.send(":ok_hand:")
                                    await ctx.send(
                                        f"**'{responses[choice]}'** was deleted"
                                    )
                                    return
                                elif choice2 == "n":
                    ...
                    if not ctx.author.bot:
                        ...
                        try:
                            ...
                            if choice2 == "y":
                                os.chdir(os.path.dirname(__file__))
                                with open(
                                    ".\\bruhbot\\responses.txt", "w", encoding="utf-8"
                                ) as f:
                                    for line in file:
                                        if (
                                            line.strip("\n").lower()
                ...
                if str(reaction.emoji) == "✅":
                    ...
                    with open(".\\bruhbot\\responses.txt", "w", encoding="utf-8") as f:
                        for line in file:
                            if (
                                line.strip("\n").lower()
                                != responses[start:end][choice].lower()
                            ):
                                f.write(line)
    ...
    try:
        ...
        if pre in args:
            ...
            with open(".\\bruhbot\\data.json") as f:
                ...
                msg = await embed(
                    ctx, helpName, helpDes, helpCont, helpColor, len(categories)
                )
...
async def age(ctx):
    ...
    year = ["year", "years"]
    await ctx.send(
        f"I am {rdelta.years} {year[plural(rdelta.years)]}, {rdelta.months} {month[plural(rdelta.months)]}, {rdelta.days} {day[plural(rdelta.days)]} old"
    )


@bot.command()
async def forza(ctx, *arg):
...
@bot.command()
@commands.is_owner()
async def logs(ctx, arg):  # so i can be lazy
    if arg == "last":
